...
## The front page has a screenshot (2026-09-21)

The picture at the top is **not** the layout probe's shot. The probe renders the real interface against canned
`/api/*` answers, which is right for measurement and wrong for a front page: its thumbnails are 2x2 PNGs
("decoding a real image is not what is being measured"), so the gallery shows empty tiles.

It is the real service, running on a **synthetic demo dataset**, screenshotted with headless Chrome:

    D:\datasets\demo-lora\           01_sky 02_dusk 03_forest 04_meadow 05_rain   (04 and 05 have no caption)
    D:\datasets\demo-lora\1_demo\    06_night 07_teal 08_amber
    D:\datasets\demo-lora\2_demo\    09_violet 10_moss

- every image is generated by a short Pillow snippet (gradients plus circles, triangles, lines and dots):
  nothing photographic, nothing from a real dataset, no watermark;
- the captions are the tags that describe those gradients (`sky, gradient, blue, clouds`), so the tag
  frequency panel has honest counts and there is nothing private in it;
- the only names on screen are the demo's own: the root appears as `demo-lora`, the subdirectories as
  `1_demo` / `2_demo`;
- the state is deliberate - two images without a caption - so the readiness strip reads
  "2 images missing a caption". It reads that way because the singular does not: "1 images missing a caption"
  is what the English pack prints, a defect this shot exposed and this round did **not** fix (a plural needs
  the i18n layer). It is written down here rather than hidden by the demo data;
- the run was `--roots D:\datasets\demo-lora` plus
