...
onnxruntime のインストールでは `--index-url` を固定しなければならない（両方のセットアップスクリプトはすでにそうしている）：グローバルの pip インデックスは固まったように見えるほど遅くなることがある（参考マシンでの実測 0.07 MB/s）。インストール後は、セッションが実際にどれを使うかを確認する：
